...
## Round 88 (2026-06) — Security Audit remediation — VERIFIED (pytest 11/11 + live curl)
Ran read-only security_audit_agent (verdict FAIL). Applied user-approved fixes:
- **SEC-001 [HIGH] PII exposure — FIXED**: deleted the unauthenticated `GET /api/contact` endpoint entirely (it exposed all leads' name/email/message with no auth; frontend never used it — leads go to inbox via Resend, Mongo is backup). Live: `GET /api/contact` → **405** (no data). Tests updated (`test_contact.py`, `backend_test.py`) to assert the endpoint is removed instead of listing leads.
- **SEC-002 [MEDIUM] form abuse — PARTIAL (per user)**: added field length caps to `ContactSubmissionCreate` (first/last ≤100, email ≤254, subject ≤200, message ≤5000, all min_length 1). Live: oversized message → **422**. (Rate-limit/CAPTCHA intentionally deferred per user choice.)
- **CORS [LOW] — FIXED at app level**: `server.py` now `allow_credentials=False` + explicit allowlist from `os.environ['CORS_ORIGINS']` (fail-fast, no `*` default). `backend/.env` CORS_ORIGINS = `https://cgreen.in,https://www.cgreen.in,https://cgreen-finance-reach.preview.emergentagent.com`. Verified directly on localhost:8001: disallowed origin → NO ACAO header; `cgreen.in` → echoed specific origin. NOTE: the preview **ingress/gateway** still injects `access-control-allow-origin: *` in front of the app (platform-level, out of app control) — the app policy is correct and governs in production.
- **PostHog form masking [LOW] — FIXED**: added `maskAllInputs: true` to `session_recording` config in `public/index.html` so contact-form (and all) input values are masked in session replays.
- **CLEAN (no action needed)**: env-only secrets (no hardcoded keys, `.env` untracked), no NoSQL injection, no XSS/`dangerouslySetInnerHTML`, external links carry `rel=noopener`, Resend reply-to validated via `EmailStr` + body HTML-escaped.
